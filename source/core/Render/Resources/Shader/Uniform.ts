...
export class Uniform {
    ...
    constructor(
        name: string,
        type: ShaderType,
        value: number | Mat4x4 | Vector3 | Vector4 | Texture2D | boolean,
        _gpuPos: WebGLUniformLocation,
        samplerNumber = -1) {
        this.name = name;
        this.type = type;
        this._value = value;
    ...
    public get Dirty() {
        return this.bDirty ||
        ((this.value instanceof Vector3 || this.value instanceof Vector4) ?
        (this.value as Vector3 | Vector4).Dirty : false);
    }
