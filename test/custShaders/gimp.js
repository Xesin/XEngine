	...
	fragmentShader:[
		...
		'vec4 blur(vec4 color, vec2 UVs, float amount) {',
			...
			'for(float i = 0.0; i < BLUR_IT; i++){',
				'finalCol += texture(texSampler, bUVs)* vertColor;',
				'bUVs.x += distance;',
			'}',
...
XEngine.GimpShader = function(){
	...
	XEngine.Material.call(this, XEngine.ShaderLib.Gimp.vertexShader, XEngine.ShaderLib.Gimp.fragmentShader, this.uniforms);
}
